...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', \
                            'D_idtA_fakeB', 'D_idtB_fakeA', \
                            'G_A_idtA_fakeB', 'G_B_idtB_fakeA', \
                            'perceptual_content_rec_A', 'perceptual_content_fake_A', 'perceptual_content_idt_B', \
                            'perceptual_content_rec_B', 'perceptual_content_fake_B', 'perceptual_content_idt_A', \
                            'perceptual_style_rec_A', 'perceptual_style_fake_A', 'perceptual_style_idt_B', \
                            'perceptual_style_rec_B', 'perceptual_style_fake_B', 'perceptual_style_idt_A', \
                            'ssim_A', 'ssim_B']
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B', 'D_idtA_fakeB', 'D_idtB_fakeA']
        ...
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, use_output_nonlinear='relu')  # 'tanh' / 'relu'
        # self.netG_A = TransformerNet(opt.input_nc, opt.output_nc).to(self.device)
        # self.netG_A.train()
        # self.netG_B = TransformerNet(opt.input_nc, opt.output_nc).to(self.device)
        # self.netG_B.train()
        if self.isTrain:
            ...
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_idtA_fakeB = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_idtB_fakeA = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:
            self.fake_A_pool = ImagePool(opt.pool_size)
            self.fake_B_pool = ImagePool(opt.pool_size)
            self.idt_A_pool = ImagePool(opt.pool_size)
            self.idt_B_pool = ImagePool(opt.pool_size)
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D_idt = torch.optim.Adam(itertools.chain(self.netD_idtA_fakeB.parameters(), self.netD_idtB_fakeA.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizers = []
            self.optimizers.append(self.optimizer_G)
            self.optimizers.append(self.optimizer_D)
            self.optimizers.append(self.optimizer_D_idt)
    ...
    def backward_D_idtA_fakeB(self):
        fake_B = self.fake_B_pool.query(self.fake_B)
        idt_A = self.idt_A_pool.query(self.idt_A)
        self.loss_D_idtA_fakeB = self.backward_D_basic(self.netD_idtA_fakeB, idt_A, fake_B)

    def backward_D_idtB_fakeA(self):
        fake_A = self.fake_A_pool.query(self.fake_A)
        idt_B = self.idt_B_pool.query(self.idt_B)
        self.loss_D_idtB_fakeA = self.backward_D_basic(self.netD_idtB_fakeA, idt_B, fake_A)
    ...
    def optimize_parameters(self):
        ...
        self.set_requires_grad([self.netD_idtB_fakeA, self.netD_idtA_fakeB], False)
        ...
        # D_idtA_fakeB and D_idtB_fakeA
        self.set_requires_grad([self.netD_idtB_fakeA, self.netD_idtA_fakeB], True)
        self.optimizer_D_idt.zero_grad()
        self.backward_D_idtB_fakeA()
        self.backward_D_idtA_fakeB()
        self.optimizer_D_idt.step()
